...
class TaskClassifier:
    ...
    # 诊断信号需要优先于“获取信息/创造设计”等通用类型，
    # 否则“我的产品没人用”会因为命中“产品”而被误判成创造设计。
    DIAGNOSIS_SIGNALS = [
        "报错", "错误", "失败", "没人用", "不工作", "不生效",
        "异常", "崩溃", "卡住", "投不出去", "坚持不下去",
        "遇到问题", "出了问题", "哪里出了问题"
    ]
    ...
    TASK_PATTERNS = {
        ...
        "问题诊断": {
            ...
            "description": "用户已经遇到一个现象或失败结果，需要定位原因并找到解决方案"
        }
    }
    ...
    def classify(self, user_input):
        ...
        # 诊断类表达通常同时命中“产品/代码/为什么”等通用词，
        # 先识别明确的失败现象，避免被其他类型抢走路由。
        diagnostic_matches = [
            signal for signal in self.DIAGNOSIS_SIGNALS
            if signal in user_input
        ]
        if diagnostic_matches:
            return {
                "task_type": "问题诊断",
                "confidence": min(0.6 + 0.2 * len(diagnostic_matches), 1.0),
                "matched_keywords": diagnostic_matches
            }
        scores = {}
